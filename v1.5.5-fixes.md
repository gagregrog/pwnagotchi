...
-   [Similar guide](https://github.com/Xyl0se/Pwnagotchi-new-guerilla-guide)
-   Maintained Forks
    -   [Pwnagotchi-Torch](https://github.com/jayofelony/pwnagotchi-torch) (Compatible w/ all Pis)
    -   [Pwnagotchi-Bookworm](https://github.com/jayofelony/pwnagotchi-bookworm) (64-bit Pis only)
...
By default your RPi won't have internet access, so either run the [internet share script](#enable-internet) or we will need to set the time manually.
...
main.plugins.haptic.debug = true
main.plugins.haptic.switch_gpio = 1
main.plugins.haptic.switch_active_high = false
```

## Add Wall of Flippers

Search for BLE enabled flippers in the vicinity by adding the Wall of Flippers service.

### Install WoF

The official Wall of Flippers app lives [here](https://github.com/K3YOMI/Wall-of-Flippers/) and a PR with pwnagotchi integration lives [here](https://github.com/K3YOMI/Wall-of-Flippers/pull/18).

To install on the pwnagotchi, connect to the pwnagotchi and then switch to root user and `/root` directory:

```sh
ssh pi@{deviceName}.local
sudo su
cd /root
```

You need internet access to install everything, so run the [network sharing script](#enable-internet).

Clone the fork of the repo, found [here](https://github.com/cyberartemio/Wall-of-Flippers), and cd into it:

```sh
git clone https://github.com/cyberartemio/Wall-of-Flippers.git && cd Wall-of-Flippers
```

Create a virtualenv, activate it, install dependencies, and deactivate it:

```sh
python3 -m venv .venv && source .venv/bin/activate && pip install -r requirements.txt && deactivate
```

If it doesn't work because you get the following error:
...
Then you can need to install `python3-venv`. Try that with:
...
Then try the updating `apt-get` with:
...
Then try udating apt-get with:

```sh
sudo apt-get update --allow-releaseinfo-change
```

And assuming that works, then try to install `venv` again:

```sh
sudo apt install python3-venv
```

Assuming _that_ works, try the origial command again:

```sh
python3 -m venv .venv && source .venv/bin/activate && pip install -r requirements.txt && deactivate
```

If that worked, yay, continue reading. If it didn't, start sleuthing.

Create a system service:

```sh
vi /etc/systemd/system/wof.service
```
...
### Install the WoF Plugin

The plugin is found [here](https://github.com/cyberartemio/wof-pwnagotchi-plugin).

Change into your custom plugins directory:
...
Fetch the zip archived plugin and move the python/assets to the plugin directory, then remove the rest:
...
Edit your config to add the following:
...
Now restart the pwnagotchi to see the flippers around you!

```sh
sudo systemctl restart pwnagotchi
```

Checkout the web ui at `http://10.0.0.2/plugins/wof`
