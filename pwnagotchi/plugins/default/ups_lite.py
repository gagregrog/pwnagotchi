...
# Updated to support UPS Lite v1.3 https://github.com/linshuqin329/UPS-Lite/blob/master/UPS-Lite_V1.3_CW2015/UPS_Lite_V1.3_CW2015.py
import logging
import struct
...
class UPS:
    def __init__(self):
        ...
        # GG - Added below to wake up the CW2015
        self._bus.write_word_data(CW2015_ADDRESS, CW2015_REG_MODE, 0x30)

        voltage = self.voltage()
        capacity = self.capacity()
        logging.info('[ups_lite] capacity: {}, voltage: {}'.format(capacity, voltage))

    def voltage(self):
        try:
            # GG - update address from 0x36 to 0x62
            address = CW2015_ADDRESS
            read = self._bus.read_word_data(address, 2)
            swapped = struct.unpack("<H", struct.pack(">H", read))[0]
            # GG - replace ( * 1.25 / 1000 / 16 )
